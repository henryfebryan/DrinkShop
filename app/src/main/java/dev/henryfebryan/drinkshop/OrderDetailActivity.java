...
import dev.henryfebryan.drinkshop.Adapter.OrderDetailAdapter;
...
public class OrderDetailActivity extends AppCompatActivity {
    ...
    private void cancelOrder() {
        ...
        drinkShopAPI.cancelOrder(String.valueOf(Common.currentOrder.getOrderId()),Common.currentUser.getPhone())
                .enqueue(new Callback<String>() {
                    @Override
                    public void onResponse(Call<String> call, Response<String> response) {
                        Toast.makeText(OrderDetailActivity.this, response.body(), Toast.LENGTH_SHORT).show();
                        if(response.body().equals("Order has been cancelled"))
                            finish();
                    }
    ...
    private void displayOrderDetail() {
        List<Cart> orderDetail = new Gson().fromJson(Common.currentOrder.getOrderDetail(), new TypeToken<List<Cart>>(){}.getType());
